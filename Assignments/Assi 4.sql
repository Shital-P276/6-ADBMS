...
-- Q1. Retrieve employee details from partition P1 and P2.
SELECT * FROM (
...
-- Q2. Retrieve employee details from partition P0 and P1 where fname begins with 'S'.
SELECT * FROM (
...
-- Q3. Count employees from each department from partitions P1, P2 and P3.
SELECT department_id, COUNT(*) AS emp_count
FROM (
    SELECT department_id FROM emp PARTITION (p1)
    UNION ALL
    SELECT department_id FROM emp PARTITION (p2)
    UNION ALL
    SELECT department_id FROM emp PARTITION (p3)
)
...
-- Q1. Retrieve sales details from 2nd partition.
SELECT *
FROM sales_hash PARTITION (h2);

-- Q2. Retrieve name of salesmen and amount from 4th partition
--     where sale amount between 2000 and 5000.
SELECT salesman_name, sales_amount
FROM sales_hash PARTITION (h4)
WHERE sales_amount BETWEEN 2000 AND 5000;

-- Q3. Find average sale amount per week from 3rd partition.
SELECT week_no, AVG(sales_amount) AS avg_sales_amount
FROM sales_hash PARTITION (h3)
GROUP BY week_no
ORDER BY week_no;